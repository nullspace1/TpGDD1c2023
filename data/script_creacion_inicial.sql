...
BEGIN TRANSACTION
	...
	ON [gd_esquema].[Maestra] (USUARIO_DNI) INCLUDE 
	(
	...
	CREATE INDEX idx_producto_nro
	ON [gd_esquema].[Maestra] (PEDIDO_NRO) INCLUDE
	(
	...
	INSERT INTO ESECUELE.DIRECCION_USUARIO (localidad_id,usuario_id,direccion)
		...
		gd_esquema.Maestra M
		JOIN
		ESECUELE.USUARIO U ON U.dni=M.USUARIO_DNI
		JOIN
	...
	INSERT INTO ESECUELE.MEDIO_DE_PAGO (tipo)
		...
		FROM
		gd_esquema.Maestra M
		JOIN ESECUELE.USUARIO U ON U.dni=M.USUARIO_DNI
	...
	INSERT INTO ESECUELE.HORARIOS_LOCAL (local_id,horario_hora_apertura,horario_hora_cierre,horario_hora_dia)
		...
		FROM
		gd_esquema.Maestra M
		JOIN ESECUELE.LOCAL L ON M.LOCAL_NOMBRE = L.nombre AND  M.LOCAL_DIRECCION = L.direccion
	...
	INSERT INTO ESECUELE.PRODUCTO (local_id,codigo,nombre,descripcion,precio_unitaro)
		...
		FROM
		gd_esquema.Maestra M
		JOIN ESECUELE.LOCAL L ON L.nombre = M.LOCAL_NOMBRE and L.descripcion = M.LOCAL_DESCRIPCION AND M.PEDIDO_NRO IS NOT NULL
	...
	INSERT INTO ESECUELE.ENVIO_MENSAJERIA (usuario_id,dir_origen,dir_destino,distancia_km,localidad_id,tipo_paquete_id,valor_asegurado,
		...
		FROM
		gd_esquema.Maestra M
		JOIN ESECUELE.USUARIO U ON  M.ENVIO_MENSAJERIA_NRO IS NOT NULL AND U.dni=M.USUARIO_DNI 
	...
	INSERT INTO ESECUELE.PEDIDO (id_pedido,usuario_id,local_id,tarjeta_id,observaciones,estado_pedido_id,fecha_pedido,calificacion,total_pedido,total_cupones)
		...
		FROM
		gd_esquema.Maestra M
		JOIN ESECUELE.ESTADO_PEDIDO PE ON PE.estado = M.PEDIDO_ESTADO AND M.PEDIDO_NRO IS NOT NULL
	...
	INSERT INTO ESECUELE.PRODUCTO_PEDIDO (id_producto,nro_pedido,cantidad,precio)
		SELECT
		PR.id_producto,
		PE.id_pedido,
		ISNULL((SELECT SUM(ISNULL(M.PRODUCTO_CANTIDAD,0)) FROM gd_esquema.Maestra M WHERE M.PEDIDO_NRO = PE.id_pedido AND M.PRODUCTO_LOCAL_CODIGO = PR.codigo),0) as cantidad,
		PR.precio_unitaro
		FROM
		ESECUELE.PEDIDO PE
		JOIN ESECUELE.PRODUCTO PR ON PR.local_id = PE.local_id
	...
	INSERT INTO ESECUELE.ENVIO (fecha_entrega,fecha_pedido,tiempo_estimado_entrega,direccion_usuario_id,tarifa_servicio,propina,repartidor_id,precio_envio,pedido_id)
		...
		FROM
		gd_esquema.Maestra M
		JOIN ESECUELE.DIRECCION_USUARIO DU ON M.DIRECCION_USUARIO_DIRECCION = DU.direccion AND M.PEDIDO_NRO IS NOT NULL
		JOIN ESECUELE.LOCALIDAD L ON DU.localidad_id = L.id_localidad
		JOIN ESECUELE.REPARTIDOR R ON R.dni=M.REPARTIDOR_DNI
	...
	INSERT INTO ESECUELE.RECLAMO (id_reclamo,usuario_id,pedido_id,tipo_reclamo_id,descr,fecha_hora,operador_id,estado_reclamo_id,solucion,fecha_hora_solucion,calificacion)
		...
		FROM
		gd_esquema.Maestra M
		JOIN ESECUELE.USUARIO U ON U.dni=M.USUARIO_DNI AND M.PEDIDO_NRO IS NOT NULL
		JOIN ESECUELE.PEDIDO P ON P.id_pedido = M.PEDIDO_NRO
...
DROP INDEX idx_producto_nro ON gd_esquema.Maestra;
GO
